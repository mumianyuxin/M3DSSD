...
__device__ inline float box_overlap_align(const float *box_a, const float *box_b){
    // params: box_a (5) [x1, y1, x2, y2, angle]
    // params: box_b (5) [x1, y1, x2, y2, angle]
    ...
    float box_a_x1=box_a_corners[0].x;
    float box_a_x2=box_a_corners[0].x;
    float box_a_y1=box_a_corners[0].y;
    float box_a_y2=box_a_corners[0].y;

    float box_b_x1=box_b_corners[0].x;
    float box_b_x2=box_b_corners[0].x;
    float box_b_y1=box_b_corners[0].y;
    float box_b_y2=box_b_corners[0].y;
    for (int k=0; k<4; k++){
        box_a_x1 = fminf(box_a_corners[k].x, box_a_x1);
        box_a_x2 = fmaxf(box_a_corners[k].x, box_a_x2);

        box_b_x1 = fminf(box_b_corners[k].x, box_b_x1);
        box_b_x2 = fmaxf(box_b_corners[k].x, box_b_x2);

        box_a_y1 = fminf(box_a_corners[k].y, box_a_y1);
        box_a_y2 = fmaxf(box_a_corners[k].y, box_a_y2);

        box_b_y1 = fminf(box_b_corners[k].y, box_b_y1);
        box_b_y2 = fmaxf(box_b_corners[k].y, box_b_y2);
    }

    float x_min_of_max = fminf(box_a_x2, box_b_x2);
    float x_max_of_min = fmaxf(box_a_x1, box_b_x1);

    float y_min_of_max = fminf(box_a_y2, box_b_y2);
    float y_max_of_min = fmaxf(box_a_y1, box_b_y1);

    if((y_min_of_max - y_max_of_min)>0 && (x_min_of_max - x_max_of_min)>0){
        return (y_min_of_max - y_max_of_min)*(x_min_of_max - x_max_of_min);
    }
    else{
        return 0;
    }
...
__device__ inline float box_convex_hull(const float *box_a, const float *box_b){
    // params: box_a (5) [x1, y1, x2, y2, angle]
    // params: box_b (5) [x1, y1, x2, y2, angle]
    ...
    float x_min=box_a_corners[0].x;
    float x_max=box_a_corners[0].x;
    float y_min=box_a_corners[0].y;
    float y_max=box_a_corners[0].y;
    for (int k=0; k<4; k++){
        x_min = fminf(box_a_corners[k].x, x_min);
        x_min = fminf(box_b_corners[k].x, x_min);

        x_max = fmaxf(box_a_corners[k].x, x_max);
        x_max = fmaxf(box_b_corners[k].x, x_max);

        y_min = fminf(box_a_corners[k].y, y_min);
        y_min = fminf(box_b_corners[k].y, y_min);

        y_max = fmaxf(box_a_corners[k].y, y_max);
        y_max = fmaxf(box_b_corners[k].y, y_max);
    }

    return fabs((x_max-x_min)*(y_max-y_min));
}
...
__global__ void boxes_overlap_vec_kernel(const int num, const float *boxes_a, const float *boxes_b, float *ans_overlap, float *ans_convex_hull){
    //const int a_idx = blockIdx.y * THREADS_PER_BLOCK + threadIdx.y;
    //const int b_idx = blockIdx.x * THREADS_PER_BLOCK + threadIdx.x;
    ...
    //float s_overlap = box_overlap_align(cur_box_a, cur_box_b);
    float s_overlap = box_overlap(cur_box_a, cur_box_b);
    float s_convex_hull = box_convex_hull(cur_box_a, cur_box_b);
    ans_overlap[idx] = s_overlap;
    ans_convex_hull[idx] = s_convex_hull;
}
...
__global__ void boxes_iou_bev_vec_kernel(const int num, const float *boxes_a, const float *boxes_b, float *ans_iou){
    //const int a_idx = blockIdx.y * THREADS_PER_BLOCK + threadIdx.y;
    //const int b_idx = blockIdx.x * THREADS_PER_BLOCK + threadIdx.x;
    const int idx = blockIdx.x * THREADS_PER_BLOCK + threadIdx.x;
...
void boxesoverlapLauncher_vec(const int num, const float *boxes_a, const float *boxes_b, float *ans_overlap, float *ans_convex_hull){

    dim3 blocks(DIVUP(num, THREADS_PER_BLOCK), 1);  // blockIdx.x(col), blockIdx.y(row)
    dim3 threads(THREADS_PER_BLOCK, 1);
    
    boxes_overlap_vec_kernel<<<blocks, threads>>>(num, boxes_a, boxes_b, ans_overlap, ans_convex_hull);
#ifdef DEBUG
    cudaDeviceSynchronize();  // for using printf in kernel function
#endif
}
...
void boxesioubevLauncher_vec(const int num, const float *boxes_a, const float *boxes_b, float *ans_iou){

    dim3 blocks(DIVUP(num, THREADS_PER_BLOCK), 1);  // blockIdx.x(col), blockIdx.y(row)
    dim3 threads(THREADS_PER_BLOCK, 1);
    
    boxes_iou_bev_vec_kernel<<<blocks, threads>>>(num, boxes_a, boxes_b, ans_iou);
}
