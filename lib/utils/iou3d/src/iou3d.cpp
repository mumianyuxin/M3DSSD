...
int boxes_overlap_bev_gpu(torch::Tensor boxes_a, torch::Tensor boxes_b, torch::Tensor ans_overlap){
...
int boxes_overlap_bev_vec_gpu(torch::Tensor boxes_a, torch::Tensor boxes_b, torch::Tensor ans_overlap, torch::Tensor ans_convex_hull){
    // params boxes_a: (N, 5) [x1, y1, x2, y2, ry]
    // params boxes_b: (N, 5) 
    // params ans_overlap: (N)
    // params ans_convex_hull: (N)
    
    CHECK_INPUT(boxes_a);
    CHECK_INPUT(boxes_b);
    CHECK_INPUT(ans_overlap);
    CHECK_INPUT(ans_convex_hull);

    int num_a = boxes_a.size(0);
    int num_b = boxes_b.size(0);
    if (num_a != num_b){
        printf( "Error! num_a !=  num_b in boxes_iou_bev_vec_gpu!!\n" );
    }

    const float * boxes_a_data = boxes_a.data<float>();
    const float * boxes_b_data = boxes_b.data<float>();
    float * ans_overlap_data = ans_overlap.data<float>();
    float * ans_convex_hull_data = ans_convex_hull.data<float>();

    boxesoverlapLauncher_vec(num_a, boxes_a_data, boxes_b_data, ans_overlap_data, ans_convex_hull_data);

    return 1;
}
int boxes_iou_bev_gpu(torch::Tensor boxes_a, torch::Tensor boxes_b, torch::Tensor ans_iou){
    // params boxes_a: (N, 5) [x1, y1, x2, y2, ry]
    // params boxes_b: (M, 5) 
    // params ans_overlap: (N, M)
...
int boxes_iou_bev_vec_gpu(torch::Tensor boxes_a, torch::Tensor boxes_b, torch::Tensor ans_iou){
    // params boxes_a: (N, 5) [x1, y1, x2, y2, ry]
    // params boxes_b: (N, 5) 
    // params ans_overlap: (N)
    ...
    boxesioubevLauncher_vec(num_a, boxes_a_data, boxes_b_data, ans_iou_data);
...
int nms_normal_gpu(torch::Tensor boxes, torch::Tensor keep, float nms_overlap_thresh){
    // params boxes: (N, 5) [x1, y1, x2, y2, ry]
    // params keep: (N)
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("boxes_overlap_bev_vec_gpu", &boxes_overlap_bev_vec_gpu, "oriented boxes vector overlap");
