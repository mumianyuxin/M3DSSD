...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, conf, root, cache_folder=None, phase='train'):
        """
        ...
        if self.phase == 'train':
            ...
            self.loader = torch.utils.data.DataLoader(self, conf.batch_size, collate_fn=self.collate, num_workers=2*conf.batch_size, pin_memory=True, shuffle=True, drop_last=True)
...
def balance_samples(conf, imdb):
    ...
    if conf.fg_image_ratio >= 0:
        ...
        for imind, imobj in enumerate(imdb):
            ...
            scale = conf.test_scale[0] / imobj.imH
