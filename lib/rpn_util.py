...
import lib.eval.kitti_common as kitti
from lib.eval.eval import get_official_eval_result
        ...
        # presuming that anchors are initialized in "2d"
            ...
            for imind, imobj in enumerate(imdb):
                ...
                if len(imobj.gts) > 0:
                    ...
                    scale = imobj.scale * conf.test_scale[0] / imobj.imH
            ...
            for gtind, gt in enumerate(normalized_gts):
                ...
                #else:
                #    print('error, gt_target_ols[gtind] =', gt_target_ols[gtind])
...
def cluster_anchors(feat_stride, anchors, test_scale, imdb, lbls, ilbls, anchor_ratios, min_gt_vis=0.99,
    ...
    for imind, imobj in enumerate(imdb):
        ...
        if len(imobj.gts) > 0:
            ...
            scale = imobj.scale * test_scale[0] / imobj.imH
...
def compute_targets_torch(gts_val, gts_ign, box_lbls, rois, fg_thresh, ign_thresh, bg_thresh_lo, bg_thresh_hi, best_thresh,
                    gts_3d=None, anchors=[], tracker=[]):
    """
    Computes the bbox targets of a set of rois and a set
    of ground truth boxes, provided various ignore
    ...
    # init transforms which respectively hold [dx, dy, dw, dh, label]
    # for labels bg=-1, ign=0, fg>=1
    transforms = torch.zeros([len(rois), 5]).cuda().float()
    raw_gt = torch.zeros([len(rois), 5]).cuda().float()

    # if 3d, then init other terms after
    if has_3d:
        transforms = F.pad(transforms, (0, gts_3d.shape[1]), 'constant')
        raw_gt = F.pad(raw_gt, (0, gts_3d.shape[1]), 'constant')
    ...
    if gts_val.shape[0] > 0 or gts_ign.shape[0] > 0:
        ...
        if gts_ign.shape[0] > 0:
            ...
            # compute overlaps ign
            ols_ign = iou_ign(rois, gts_ign)
            ols_ign_max = torch.max(ols_ign, dim=1)[0]

        else:
            ols_ign_max = torch.zeros([rois.shape[0]]).cuda().float()

        if gts_val.shape[0] > 0:

            # compute overlaps valid
            ols = iou(rois, gts_val)
            ols_max = torch.max(ols, dim=1)[0]
            targets = torch.argmax(ols, dim=1)

            # find best matches for each ground truth
            gt_best_rois = torch.argmax(ols, dim=0)
            gt_best_ols = torch.max(ols, dim=0)[0]

            gt_best_rois = gt_best_rois[gt_best_ols >= best_thresh]
            gt_best_ols = gt_best_ols[gt_best_ols >= best_thresh]

            fg_inds = torch.nonzero(torch.flatten(ols_max) >= fg_thresh)
            fg_inds = fg_inds.squeeze()
            fg_inds = torch.cat((fg_inds, gt_best_rois))
            fg_inds = torch.unique(fg_inds)
            ...
            if len(fg_inds) > 0:
                ...
                if has_3d:
                    ...
                    tracker = tracker.type(torch.int64)
                    #anchors = torch.from_numpy(anchors).cuda().float()
                    src_3d = anchors[tracker[fg_inds], 4:]
                    gts_3d = torch.from_numpy(gts_3d).cuda().float()
                    target_3d = gts_3d[targets[fg_inds]]
                ...
                # store labels
                transforms[fg_inds, 4] = torch.tensor([box_lbls[x] for x in targets[fg_inds]], dtype=torch.float)
                assert (all(transforms[fg_inds, 4] >= 1))

        else:

            ols_max = torch.zeros(rois.shape[0], dtype=int)
            fg_inds = torch.empty(shape=[0])
            gt_best_rois = torch.empty(shape=[0])

        # determine ignores
        ign_inds = torch.nonzero(ols_ign_max >= ign_thresh)
        ign_inds = ign_inds.squeeze()

        # determine background
        bg_inds = torch.nonzero((ols_max >= bg_thresh_lo) & (ols_max < bg_thresh_hi))
        bg_inds = bg_inds.squeeze()

        # subtract fg and igns from background
        time_1 = time()
        bg_inds  = bg_inds.cpu().numpy()
        ign_inds = ign_inds.cpu().numpy()
        fg_inds  = fg_inds.cpu().numpy()
        gt_best_rois = gt_best_rois.cpu().numpy()
        bg_inds = np.setdiff1d(bg_inds, ign_inds)
        bg_inds = np.setdiff1d(bg_inds, fg_inds)
        bg_inds = np.setdiff1d(bg_inds, gt_best_rois)

        time_2 = time()
        #print('set diff 1d time:{:.4}'.format(time_2-time_1) )
        # mark background
        transforms[bg_inds, 4] = -1
    ...
    if (cache_folder is not None) and os.path.exists(os.path.join(cache_folder, 'bbox_means.pkl')) \
        ...
        for imind, imobj in enumerate(imdb):
            ...
            if len(imobj.gts) > 0:
                ...
                scale_factor = imobj.scale * conf.test_scale[0] / imobj.imH
...
def bbox_transform_3d(ex_rois_2d, ex_rois_3d, gt_rois):
    ...
    if type(gt_rois) == torch.Tensor:
        scale_w = torch.log(gt_rois[:, 3] / ex_rois_3d[:, 1])
        scale_h = torch.log(gt_rois[:, 4] / ex_rois_3d[:, 2])
        scale_l = torch.log(gt_rois[:, 5] / ex_rois_3d[:, 3])
        deltaRotY = gt_rois[:, 6] - ex_rois_3d[:, 4]

        targets = torch.stack((targets_dx, targets_dy, delta_z, scale_w, scale_h, scale_l, deltaRotY), dim=1)
        targets = torch.cat((targets, gt_rois[:, 7:]), dim=1)
    else:
...
def bbox_transform(ex_rois, gt_rois):
    ...
    targets_dx = (gt_ctr_x - ex_ctr_x) / ex_widths
    targets_dy = (gt_ctr_y - ex_ctr_y) / ex_heights
    if type(gt_widths) == torch.Tensor:
...
def bbox_transform_inv(boxes, deltas, means=None, stds=None):
    ...
    pred_boxes = torch.zeros(deltas.shape, device=boxes.device)
...
def bbox_transform_inv_new(boxes, deltas, means=None, stds=None):
    """
    Compute the bbox target transforms in 3D.
    ...
    if len(boxes.shape) == 2:
        ...
        pred_boxes = torch.zeros(deltas.shape, device=deltas.device)
...
def locate_position(feat_size, stride, convert_tensor=False, device='cuda'):

    shift_x = np.array(range(0, feat_size[1], 1)) * float(stride)
    shift_y = np.array(range(0, feat_size[0], 1)) * float(stride)

    # scale to (0, 1)
    shift_x = shift_x / (feat_size[1]*stride)
    shift_y = shift_y / (feat_size[0]*stride)

    [shift_x, shift_y] = np.meshgrid(shift_x, shift_y)

    mean_x = np.mean(shift_x)
    std_x  = np.std(shift_x)

    mean_y = np.mean(shift_y)
    std_y  = np.std(shift_y)

    shift_x = (shift_x-mean_x) / std_x
    shift_y = (shift_y-mean_y) / std_y

    pos = np.stack((shift_x, shift_y), axis=0)
    if convert_tensor:
...
def im_detect_3d(im, net, rpn_conf, obj, gpu=0, synced=False):
    """
    ...
    bbox_y3d = bbox_y3d * bbox_stds[0, 5] + bbox_means[0, 5]
    bbox_z3d = bbox_z3d * bbox_stds[0, 6] + bbox_means[0, 6]
    bbox_w3d = bbox_w3d * bbox_stds[0, 7] + bbox_means[0, 7]
    bbox_h3d = bbox_h3d * bbox_stds[0, 8] + bbox_means[0, 8]
    bbox_l3d = bbox_l3d * bbox_stds[0, 9] + bbox_means[0, 9]
    bbox_ry3d = bbox_ry3d * bbox_stds[0, 10] + bbox_means[0, 10]

    # find 3d source
    #tracker = rois[:, 4].cpu().detach().numpy().astype(np.int64)
    tracker = rois[:, 4].long()
    src_3d = anchors[tracker, 4:].type(torch.cuda.FloatTensor)
    ...
    #coords_3d[:, 2] *= scale_factor

    cls_pred = torch.argmax(prob[:, 1:], dim=1) + 1
    scores, _ = torch.max(prob[:, 1:], dim=1)
    ...
    tracker = tracker[sorted_inds].float()

    if synced:

        # nms
        keep_inds = gpu_nms(aboxes[:, 0:5].type(torch.float), rpn_conf.nms_thres, device_id=gpu)

        # convert to bool
        keep = torch.zeros([aboxes.shape[0], 1], dtype=bool).cuda()
        keep[keep_inds, :] = True

        # stack the keep array,
        # sync to the original order
        #aboxes = np.hstack((aboxes, keep))
        aboxes = torch.cat((aboxes, keep), dim=1)
        ...
        # nms
        keep_inds = gpu_nms(aboxes[:, 0:5].detach().cpu().numpy().astype(np.float32), rpn_conf.nms_thres, device_id=gpu)

        # stack cls prediction
        aboxes = torch.cat((aboxes, cls_pred[:, np.newaxis], coords_3d, tracker[:, np.newaxis]), dim=1)

        # suppress boxes
        aboxes = aboxes[keep_inds, :]

    aboxes = aboxes.cpu().numpy()
    # clip boxes
...
def im_detect_3d_acc(im, net, rpn_conf, obj, gpu=0, synced=False):
    """
    Object detection in 3D
    """
    time_1 = time()
    ...
    scores, _ = torch.max(prob[:, 1:], dim=1)
    _, sorted_inds = torch.sort(scores, descending=True)

    if not synced:
        num = min(rpn_conf.nms_topN_pre, cls_pred.shape[0])
    else:
        num = cls_pred.shape[0]

    sorted_inds = sorted_inds[0:num]

    # sort all output
    bbox_2d = bbox_2d.squeeze(0).detach()
    bbox_3d = bbox_3d.squeeze(0).detach()

    misc = torch.cat((bbox_2d, bbox_3d, rois, cls_pred.unsqueeze(1), scores.unsqueeze(1)), dim=1)
    misc = misc[sorted_inds]

    ind = 0
    bbox_2d = misc[:, 0:bbox_2d.shape[1]]
    ind += bbox_2d.shape[1]

    bbox_3d = misc[:, ind:ind+bbox_3d.shape[1]]
    ind += bbox_3d.shape[1]

    rois = misc[:, ind:ind+rois.shape[1]]
    ind += rois.shape[1]

    cls_pred= misc[:, ind:ind+1]
    ind += 1

    scores  = misc[:, ind:ind+1]
    ind += 1
    assert ind == misc.shape[1], 'ind={}, misc.shape={}'.format(ind, misc.shape)


    # compute feature resolution
    num_anchors = rpn_conf.anchors.shape[0]

    std = torch.from_numpy(rpn_conf.bbox_stds[:, 4:11]).float().cuda()
    mean =  torch.from_numpy(rpn_conf.bbox_means[:, 4:11]).float().cuda()
    bbox_3d *= std
    bbox_3d += mean

    # find 3d source
    tracker = rois[:, 4].int()
    src_3d = torch.from_numpy(rpn_conf.anchors[tracker, 4:]).float().cuda()
    ...
    bbox_3d[:, 0] = bbox_3d[:, 0] * widths + ctr_x
    bbox_3d[:, 1] = bbox_3d[:, 1] * heights + ctr_y
    bbox_3d[:, 2] = bbox_3d[:, 2] + src_3d[:, 0]
    bbox_3d[:, 3] = torch.exp(bbox_3d[:, 3]) * src_3d[:, 1]
    bbox_3d[:, 4] = torch.exp(bbox_3d[:, 4]) * src_3d[:, 2]
    bbox_3d[:, 5] = torch.exp(bbox_3d[:, 5]) * src_3d[:, 3]
    bbox_3d[:, 6] = bbox_3d[:, 6] + src_3d[:, 4]
    ...
    #aboxes = np.hstack((coords_2d, scores[:, np.newaxis]))
    aboxes = torch.cat((coords_2d, scores, cls_pred,
                        coords_3d, tracker.float().unsqueeze(1)), dim=1)
    ...
    if synced:
        ...
        # sync to the original order
        aboxes = np.hstack((aboxes, keep))
        _, original_inds = torch.sort(sorted_inds)
        aboxes[original_inds, :]
        ...
        # nms
        keep_inds = gpu_nms(aboxes[:, 0:5].cpu().numpy(), rpn_conf.nms_thres, device_id=gpu)
...
def test_kitti_3d(dataset_test, net, rpn_conf, results_path, test_path, use_log=True, writer=None, phase='validation'):
    """
    ...
    #for imind, batch in enumerate(dataset_test):
    #for batch in tqdm(dataset_test):
    with tqdm(dataset_test) as t_test:
        for batch in t_test:
            if rpn_conf.pre_compute_target:
                im = batch['input']
                imobj = edict(batch['target']['meta'])
            else:
                im, imobj = batch
            ...
            # read in calib
            #p2 = read_kitti_cal(os.path.join(test_path, dataset_test, 'validation', 'calib', imobj.id + '.txt'))
            p2 = imobj.p2
            p2_inv = np.linalg.inv(p2)
            # forward test batch
            #aboxes, stats = im_detect_3d_acc(im, net, rpn_conf, imobj)
            aboxes = im_detect_3d(im, net, rpn_conf, imobj)

            #base_path, name, ext = file_parts(impath)
            time_1 = time()
            file = open(os.path.join(results_path, imobj.id + '.txt'), 'w')
            text_to_write = ''
            ...
            time_2 = time()
            #stats['post'] = time_2 - time_1
            #t_test.set_postfix(stats)
            # display stats
            #if (imind + 1) % 1000 == 0:
            #    time_str, dt = compute_eta(test_start, imind + 1, len(dataset_test))

            #    print_str = 'testing {}/{}, dt: {:0.3f}, eta: {}'.format(imind + 1, len(dataset_test), dt, time_str)

            #    if use_log: logging.info(print_str)
            #    else: print(print_str)

    # evaluate
    #val_image_ids = list(range(len(dataset_test)))
    if phase == 'validation':
        gt_path = os.path.join(test_path, rpn_conf['datasets_{}'.format(phase)][0].name, 'validation', 'label_2')
    elif phase == 'train':
        gt_path = os.path.join(test_path, rpn_conf['datasets_{}'.format(phase)][0].name, 'training', 'label_2')
    elif phase == 'test':
        gt_path = os.path.join(test_path, rpn_conf['datasets_{}'.format(phase)][0].name, 'testing', 'label_2')

    dt_annos = kitti.get_label_annos(results_path)
    gt_annos = kitti.get_label_annos(gt_path)
    #print(get_official_eval_result(gt_annos, dt_annos, 0)) # 6s in my computer
    res, res_stats = get_official_eval_result(gt_annos, dt_annos, [0, 1, 2]) # 6s in my computer

    if writer is not None:
        items = ['aos', '3d', 'bev', 'image']
        for lbl in rpn_conf.lbls:
            for item in items:
                easy = res_stats['{}_{}_easy'.format(lbl, item)]
                mod = res_stats['{}_{}_moderate'.format(lbl, item)]
                hard = res_stats['{}_{}_hard'.format(lbl, item)]
                if item == '3d' and lbl == 'Car':
                    easyR40 = res_stats['{}_{}_easy_R40'.format(lbl, item)]
                    modR40  = res_stats['{}_{}_moderate_R40'.format(lbl, item)]
                    hardR40 = res_stats['{}_{}_hard_R40'.format(lbl, item)]
                    writer.add_scalars("Test/{}_{}".format(lbl, item), {'easyR40':easyR40, 'modR40':modR40, 'hardR40':hardR40}, test_iter)
                    iou_3d.append(easyR40)
                    iou_3d.append(modR40)
                    iou_3d.append(hardR40)

                writer.add_scalars("Test/{}_{}".format(lbl, item), {'easy':easy, 'mod':mod, 'hard':hard}, test_iter)

    if use_log: logging.info(res)
    else: print(res)

    '''
    if val_train:
        script = os.path.join(test_path, rpn_conf.datasets_test[0]['name'], 'devkit', 'cpp', 'evaluate_train')
    else:
        script = os.path.join(test_path, rpn_conf.datasets_test[0]['name'], 'devkit', 'cpp', 'evaluate_object')
    print(script)
    ...
    for lbl in rpn_conf.lbls:
        ...
        if os.path.exists(respath_2d):
            ...
            print_str = 'test_iter {} 2d {} --> easy: {:0.4f}, mod: {:0.4f}, hard: {:0.4f}'.format(test_iter, lbl,
                                                                                                    easy, mod, hard)
            if writer is not None:
                writer.add_scalars("Test/2D_{}_IOU".format(lbl), {'easy':easy, 'mod':mod, 'hard':hard}, test_iter)
            #writer.add_scalar("Test/2D_{}_IOU/mod".format(lbl), mod, test_iter)
            #writer.add_scalar("Test/2D_{}_IOU/hard".format(lbl), hard, test_iter)
            if use_log: logging.info(print_str)
            else: print(print_str)
        ...
        if os.path.exists(respath_gr):
            ...
            if writer is not None:
                writer.add_scalars("Test/BEV_{}_IOU".format(lbl), {'easy': easy, 'mod':mod, 'hard':hard}, test_iter)
            #writer.add_scalar("Test/BEV_{}_IOU/easy".format(lbl), easy, test_iter)
            #writer.add_scalar("Test/BEV_{}_IOU/mod".format(lbl), mod, test_iter)
            #writer.add_scalar("Test/BEV_{}_IOU/hard".format(lbl), hard, test_iter)
            if use_log: logging.info(print_str)
            else: print(print_str)
        ...
        if os.path.exists(respath_3d):
            ...
            if writer is not None:
                writer.add_scalars("Test/3D_{}_IOU".format(lbl), {'easy':easy, 'mod':mod, 'hard':hard}, test_iter)
            #writer.add_scalar("Test/3D_{}_IOU/easy".format(lbl), easy, test_iter)
            #writer.add_scalar("Test/3D_{}_IOU/mod".format(lbl), mod, test_iter)
            #writer.add_scalar("Test/3D_{}_IOU/hard".format(lbl), hard, test_iter)
            iou_3d.append([easy, mod, hard])
            if use_log: logging.info(print_str)
            else: print(print_str)
    '''
    #return iou_3d, outputs
    return iou_3d
